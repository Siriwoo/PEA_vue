<template>
  <div class="app-container historyreportcss">
  <div class="viewmenu" v-if="showview">
    <el-row :gutter="10">
      ...
      <el-col :xs="11" :sm="4" :lg="4">
        ...
        <div class="">
          <el-select v-model="seltype" :disabled="checkedAlltypehis" filterable placeholder="Select">
            <el-option
        ...
        <div class="buttun_text">
          ประเภทประวัติ หรือ (<el-checkbox v-model="checkedAlltypehis">ทั้งหมด</el-checkbox>)
        </div>
      </el-col>
      <el-col :xs="20" :sm="16" class="suitemo" v-if="seloptionhis=='เจาะจงอุปกรณ์'">
        <div class="" >
      ...
      <el-col :xs="20" :sm="9" class="suitemo" v-if="seloptionhis=='ภาพรวม'">
        <div class="lo_n_date_eq" >
          <div class="">
            <div class="">
              <el-select v-model="sellocation"  :disabled="checkedAlllocation" filterable placeholder="เลือกสถานที่">
                <el-option
            ...
            <div class="buttun_text">
              เลือกสถานที่ หรือ (<el-checkbox v-model="checkedAlllocation">ทุกที่</el-checkbox>)
            </div>
          </div>
          <div class="" >
            <div class="">
              <el-select v-model="seltypeequipt"  :disabled="checkedAllequipt" filterable placeholder="เลือกอุปกรณ์">
                <el-option
            ...
            <div class="buttun_text">
              เลือกอุปกรณ์ หรือ (<el-checkbox v-model="checkedAllequipt">ทุกอุปกรณ์</el-checkbox>)
            </div>
          </div>
        </div>

      </el-col>
      <el-col :xs="20" :sm="8" class="suitemo" v-if="seloptionhis=='ภาพรวม'">
        <div class="lo_n_date_eq" >
          <div class="mb-1">
            <el-date-picker
              v-model="sstart"
          ...
          <div class="buttun_text">
            เริ่มต้น-สิ้นสุด หรือ
            (<el-radio v-model="checkdate" label="tyear" @keyup.enter.native="searchall">ปีนี้</el-radio>
            <el-radio v-model="checkdate" label="tmonth" @keyup.enter.native="searchall">เดือนนี้</el-radio>
            <el-radio v-model="checkdate" label="tweek" @keyup.enter.native="searchall">สัปดาห์นี้</el-radio>)
          </div>
      ...
      <el-col :xs="24" :sm="24">
        <el-row v-if="seltype!='ข้อมูล'">
          ...
          <el-col :xs="12" :sm="4"
          v-if="!checkedAlltypehis&&optionhis!=='ภาพรวม'&&typeuser=='admin'&&seltype!='ข้อมูล'" v-bind:style= "[(typepage=='เพิ่ม') ? bgtopnav:bgtopnavn]">
       ...
       <span slot="salary" slot-scope="{row}">
        ...
        <div slot="Operations" slot-scope="{row}">
          <div class="operation-style">
            ...
            <span v-if="typeof tableData[row.ลำดับ-1].detail !=='undefined'">
            <span v-if="whoplay.empcode==tableData[row.ลำดับ-1].detail.whoCreate">
...
export default {
  ...
  data() {
    return {
      ...
      seltypeequipt: '',
      equiptoptions: [],
      checkedAlltypehis: false,
      checkedAllequipt: false,
      checkedAlllocation: false,
      checksearch: '',
      readyform: false,
      refreshformreport: 0,
      columns: [],
      ...
      allkey: [],
      dallkey: ['อุปกรณ์','ยี่ห้อ','รุ่น','สถานที่'],
  ...
  watch: {
    ...
    checkdate(v){
      ...
      this.send = new Date()
      if(v=='tyear'){
        this.sstart = new Date(new Date().getFullYear(), 0, 1);
      }else if (v=='tmonth') {
        this.sstart = new Date(this.send.getFullYear(), this.send.getMonth(), 1);
      }else if (v=='tweek') {
        this.sstart = this.getMonday(new Date())
    ...
    seloptionhis(v){
      this.tableData = []
      console.log(v);
      if(v==='ช่วงเวลา'||v==='สถานที่'||v==='ภาพรวม'){
        this.optionst = this.optionst.filter(e=>{
          return e.value !== 'ข้อมูล'
        })
        this.checkedAlltypehis = true
      }else{
    ...
    equiptall(v){
      if(v.length>0){
        console.log('focusequit in equiptall');
        console.log(v);
  ...
  mounted() {
    ...
    this.whoplay  = JSON.parse(getUserData())
  },
  methods: {
    searchnow(){
      this.fillinputform(this.focusequit)
    ...
    fillinputform(d){
      console.log('value in fillinputform method');
      console.log(d)
      if(this.seltype =='ข้อมูล'){
        ...
        if(typeof d.detail !== 'undefined'){
          if(typeof JSON.parse(d.detail.infodetail) !== 'undefined'){
            ...
            this.whoCreate.name = d.detail.name
            this.whoCreate.prename = d.detail.prename
            this.whoCreate.lastname = d.detail.lastname
            this.whoCreate.picprofile = d.detail.picprofile
            this.whoCreate.timecreate = d.detail.timecreate_c
          }
        }else{
    ...
    getData2(){
      if(true){
        ...
        if(typeof JSON.parse(getUserData()).depart !=='undefined' ){
          ...
          var flagvalid = true
          if(this.seloptionhis==='เจาะจงอุปกรณ์'){
            t = {
          ...
          }else if (this.seloptionhis==='ภาพรวม') {
            ...
            t = {
              ...
              sstart:this.sstart,
              send:this.send,
              alllocation:this.checkedAlllocation,
              locationID:this.sellocation,//,
              allequipt:this.checkedAllequipt,
              equiptID:this.seltypeequipt,
              typeHistory:this.seltype
            }
          ...
          }////ช่วงเวลา กับ สถานที่ รอลบออกต่อไป
          if(flagvalid){
            t.seloptionhis = this.seloptionhis
            t.checkedAlltypehis = this.checkedAlltypehis
            var tmp = {}
                ...
                .then((r) => {
                  ...
                  try {
                    ...
                    this.tableData = r.data.map(obj=>{
                      ...
                      const found = this.equiptype.find(e => e.id == obj.detail.equiptypeID);
                      //console.log(found);
                      rObj.ลำดับ = k
                      rObj.ประเภทอุปกรณ์ = found.equip
              ...
              .then((r) => {
                ...
                if(typeof JSON.parse(getUserclick()).seloptionhis !== 'undefined'){
                  ...
                  if(this.seloptionhis == 'อุปกรณ์'){
                    this.seloptionhis = 'เจาะจงอุปกรณ์'
                  }
                }else{
                  var tmp = JSON.parse(getUserclick())
                  tmp.seloptionhis = ''
        ...
        try {
          ...
          this.tableData = this.ortableData.filter(e=>{
            ...
            if(this.checksearch=='not'){
              //console.log(this.forsearchdevice.toUpperCase());
              if(!astext.includes(this.forsearchdevice.toUpperCase())){
                return e
              }
            }else{
