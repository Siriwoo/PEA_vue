    ...
    <v-card>
      <span v-for="(elem,i) in wanttag_sq" :key="i">
        <v-chip v-model="elem.show">{{elem.name}}
          <i class="el-icon-circle-close custom-icon" @click="d_wanttag(i)"></i>
        </v-chip>
      </span>
...
<!--1111-->
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" round small fab @click="addAsk">
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" round small fab @click="EditAsk"><v-icon>save</v-icon></v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
/* eslint-disable */
  ...
  import dAns from './d_ans_setting.vue'
  export default{
    data () {
      return {
    ...
    methods: {
      ...
      c_bottombuttom: function (d) {
        this.$store.dispatch('chatbot/c_bottombuttom', d)
      },
      S_Intent: function (d) {
        this.$store.dispatch('chatbot/S_Intent', d)
      },
      ...
      editAnswer: function (val, j) {
        ...
        for (var i = 0; i < valtmp.length; i++) {
          ...
          if (valtmp[i].type === 'text' || valtmp[i].type === 'Text') {
            ...
            valtmp[i].type_add = 'text'
          } else if (valtmp[i].type === 'sticker') {
            valtmp[i].label = 'สติ๊กเกอร์'
            valtmp[i].type_add = 'sticker'
          } else if (valtmp[i].type === 'location') {
