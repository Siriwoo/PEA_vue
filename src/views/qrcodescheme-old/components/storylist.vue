...
export default {
  ...
  methods: {
    ...
    loadallhistoy(start,end){
      ...
      tmp.param1 = JSON.parse(getUserclick()).viewscheme.office
      tmp.param2 = {
        start:start,
        end:end
      }
    ...
    convertliststory_to_calendar(l){
      ...
      return l.map(e=>{
        ...
        new_e.name = `${ae.typeHistory},${ae.focuswarehouse.name1}`
        if(ae.typeHistory=='ย้าย'){
          new_e.color = 'blue'
        }else if (ae.typeHistory=='เพิ่ม') {
          new_e.color = 'indigo'
        }else if (ae.typeHistory=='ลด') {
          new_e.color = 'cyan'
        }
        var first = ''
        var second = ''
