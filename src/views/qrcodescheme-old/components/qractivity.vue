<template>
        ...
        <div class="box-card box-card-nav">
          <div class="basefocus" v-bind:class="{ 'focus': e.isfocus }" v-for="(e,index) in toolfromqr">
            ...
            <div class="dis_input">
              ...
              <div class="pb">
                <div class="">
                  /<span v-if="schemedo.name=='ย้าย'||schemedo.name=='ลด'">{{e.nownum}}>{{e.nownum-toolfromqr[index].tonum}} </span>
                  <span v-if="schemedo.name=='เพิ่ม'">{{e.nownum}}>{{e.nownum+toolfromqr[index].tonum}} </span>
                    {{e.unit}}
                  <span v-if="schemedo.name=='ย้าย'"> ไปยัง {{toolfromqrdest[index].nownum+toolfromqr[index].tonum}}/{{toolfromqrdest[index].nownum}} {{e.unit}}</span>
                </div>
                <div class="bex">
                  จำนวนคงเหลือ
                </div>
  ...
  <el-row style="padding:5px 5px 5px;" align="top">
    ...
    v-if="isadmin()"
    :loading="isloadsave" @click="entermasterscheme_qr()">บันทึกโครงการ</el-button>
  </el-row>
  <el-dialog
    :title="name_select"
